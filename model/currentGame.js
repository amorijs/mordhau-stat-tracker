...
module.exports.update = (scoreboard, roundsUpdate) => {
    ...
    scoreboardArray.forEach((row) => {
        ...
        if (playfab === "There are currently no players present.") return;
        ...
        const assists = Number.parseInt(
            splitByCommas[splitByCommas.length - 1]
        );
    ...
    const promises = scoreboardEntries.map(
        async ([playfab, { score, kills, deaths, assists, name }]) => {
            ...
            const newRoundsPlayed =
                playerToUpdate.roundsPlayed +
                currentGame.roundsWonByTeam0 +
                currentGame.roundsWonByTeam1;
